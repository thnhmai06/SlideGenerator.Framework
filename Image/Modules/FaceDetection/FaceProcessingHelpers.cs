...
/// </summary>
/// <remarks>This class is intended for internal use in facial analysis workflows and is not thread-safe. All
...
/// within expected ranges.</remarks>
internal static class FaceProcessingHelpers
{
    ...
    /// </summary>
    /// <remarks>If no faces are provided, the method returns <see langword="false"/> and sets <paramref
    /// name="center"/> to the default value. When eye center data is unavailable, the method uses a fallback anchor
    /// point based on the provided ratio.</remarks>
    /// <param name="faces">A read-only list of detected faces to analyze for eye centroid calculation. Must not be empty.</param>
    /// <param name="useAllFaces">Indicates whether to use all faces in the collection for averaging the eye centroid. If <see langword="false"/>,
    /// only the best face is used.</param>
    /// <param name="eyeLineRatio">The ratio used to determine the fallback anchor point along the eye line if eye center data is unavailable. Must
    /// be a value between 0.0 and 1.0.</param>
    /// <param name="center">When this method returns, contains the calculated centroid point between the eyes if successful; otherwise,
    /// contains a fallback anchor point.</param>
    ...
    /// </summary>
    /// <remarks>If the <paramref name="faces"/> collection contains only one element, its rectangle is
    /// returned. The method does not validate that the rectangles are non-empty or non-overlapping.</remarks>
    /// <param name="faces">A read-only list of <see cref="Face"/> objects whose rectangles will be combined into a single bounding
    /// rectangle. Must contain at least one element.</param>
    ...
    /// </summary>
    /// <remarks>This method assumes that the input list contains at least one face. No validation is
    /// performed on the input; passing an empty list will result in an exception.</remarks>
    ...
    /// </summary>
    /// <remarks>Faces without a valid eye center are ignored in the calculation. If no faces contain a valid
    /// eye center, the method returns false and sets the output parameter to its default value.</remarks>
    /// <param name="faces">A read-only list of face objects to analyze for eye center positions. Each face is evaluated for a valid eye
    /// center.</param>
    /// <param name="center">When this method returns, contains the mean center point of all eyes found if successful; otherwise, the default
    /// value.</param>
    ...
    /// </summary>
    /// <remarks>This method is useful for facial feature alignment tasks, such as overlaying graphics or
    /// performing further analysis based on eye position. The anchor point is calculated using integer rounding for the
    /// vertical position.</remarks>
    /// <param name="face">The rectangle representing the detected face area. The anchor point will be calculated relative to this region.</param>
    /// <param name="eyeLineRatio">The relative vertical position of the eye line within the face rectangle, expressed as a ratio between 0.0 (top)
    /// and 1.0 (bottom). Must be between 0.0 and 1.0.</param>
    /// <returns>A Point representing the anchor position on the eye line, horizontally centered within the face rectangle and
    /// vertically positioned according to the specified ratio.</returns>
