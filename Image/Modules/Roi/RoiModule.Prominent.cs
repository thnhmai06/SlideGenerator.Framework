...
public sealed partial class RoiModule
{
    ...
    private ValueTask<Rectangle> GetProminentRoiAsync(Image.Models.Image image, Size size)
    {
    ...
    /// <param name="size">The target size that defines the base crop dimensions.</param>
    /// <returns>A Rectangle representing the most prominent region of interest within the image.</returns>
    private Rectangle GetProminentRoi(Image.Models.Image image, Size size)
    {
        using var saliencyMap = ImageUtilities.ComputeSaliency(image);
        ...
        // expand roi
        roi = Options.SaliencyPaddingRatio.With(roi, new Rectangle(0, 0, w, h));
        return roi;
    }
}